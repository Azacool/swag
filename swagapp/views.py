...
class Purchase(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        data = list(Purchase.objects.all().values())
        return Response(data)

    def retrieve(self, request, *args, **kwargs):
        data = list(Purchase.objects.filter(id=kwargs['pk']).values())
        return Response(data)

    def create(self, request, *args, **kwargs):
        purchase_serializer_data = PurchaseSerializer(data=request.data)
        if purchase_serializer_data.is_valid():
            purchase_serializer_data.save()
            status_code = status.HTTP_201_CREATED
            return Response({"message": "Purchase added successfully", "status": status_code})
        else:
            status_code = status.HTTP_400_BAD_REQUEST
            return Response({"message": "Please fill in the details", "status": status_code})

    def destroy(self, request, *args, **kwargs):
        purchase_data = Purchase.objects.filter(id=kwargs['pk'])
        if purchase_data:
            purchase_data.delete()
            status_code = status.HTTP_201_CREATED
            return Response({"message": "Purchase deleted successfully", "status": status_code})
        else:
            status_code = status.HTTP_400_BAD_REQUEST
            return Response({"message": "Purchase data not found", "status": status_code})

    def update(self, request, *args, **kwargs):
        purchase_details = Purchase.objects.get(id=kwargs['pk'])
        purchase_serializer_data = PurchaseSerializer(
            purchase_details, data=request.data, partial=True)
        if purchase_serializer_data.is_valid():
            purchase_serializer_data.save()
            status_code = status.HTTP_201_CREATED
            return Response({"message": "Purchase updated successfully", "status": status_code})
        else:
            status_code = status.HTTP_400_BAD_REQUEST
            return Response({"message": "Purchase data not found", "status": status_code})
